 ...
 */
	...
	public function __construct($po_response, $pa_options=null) {
		...
		$this->ops_path_info = isset($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : $_SERVER['ORIG_PATH_INFO'];
	}
	# -------------------------------------------------------
	/** 
	...
	public function getUILocales() {
		...
		if (sizeof($va_locale_codes) == 0) {
			foreach($t_locale->getLocaleList() as $vn_locale_id => $va_locale_info) {
 ...
 */	
	public function doAuthentication($pa_options) {	
		...
		if (!$vb_login_successful) {	
			...
			if (!$pa_options["dont_redirect_to_login"]) {
				$vs_auth_login_url = $this->getBasePath().'/'.$this->getScriptName().'/'.$this->config->get("auth_login_path");
			...
			if (!$pa_options["dont_redirect_to_welcome"]) {
				//header("Location: ".$this->getBasePath().'/'.$this->getScriptName().'/'.$this->config->get("auth_login_welcome_path"));				// redirect to "welcome" page
				$this->opo_response->addHeader("Location", $this->getBasePath().'/'.$this->getScriptName().'/'.$this->config->get("auth_login_welcome_path"));
				//exit;
			}
