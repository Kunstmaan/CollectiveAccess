...
class WLPlugMediaXMLDoc Extends WLPlug Implements IWLPlugMedia {
	...
	# ------------------------------------------------
	public function htmlTag($ps_url, $properties, $pa_options=null, $pa_volume_info=null) {
		...
		if(preg_match("/\.xml\$/", $ps_url)) {
			...
			if(preg_match("/\.pdf\$/", $ps_url)) {
				...
				if (($ps_url) && ($properties["width"] > 0) && ($properties["height"] > 0)) {
				
					return "<img src='$ps_url' width='".$properties["width"]."' height='".$properties["height"]."' border='$border' $vspace $hspace $alt $title $name $usemap $align $class $style />";
				} else {
