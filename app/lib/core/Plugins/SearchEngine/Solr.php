...
class WLPlugSearchEngineSolr extends WLPlug implements IWLPlugSearchEngine {
	# -------------------------------------------------------
	...
	public function init(){
		...
		$this->ops_encoding = $this->opo_app_config->get('character_set');
	...
	public function search($pn_subject_tablenum, $ps_search_expression, $pa_filters=array(), $po_rewritten_query=null){
		if ($vs_filter_query = $this->_filtersToLuceneQuery($pa_filters)) {
		...
		return new WLPlugSearchEngineSolrResult($va_result["response"]["docs"], array());
	}
	# -------------------------------------------------------
	private function _filtersToLuceneQuery($pa_filters) {
		$va_terms = array();
