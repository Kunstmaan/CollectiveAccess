<?php
/** ---------------------------------------------------------------------
 * app/lib/core/Plugins/SearchEngine/MysqlFulltextResult.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class WLPlugSearchEngineMysqlFulltextResult extends WLPlug implements IWLPlugSearchEngineResult {
	# -------------------------------------------------------
	private $opo_config;
	...
	public function setHits($pa_hits) {
		...
		if (sizeof($this->opa_hits)) {
			...
			$this->opo_subject_instance = $this->opo_datamodel->getInstanceByTableNum($va_tmp[0], true);
	...
	public function get($ps_field, $pa_options=null) {
		...
		$vs_val = false;	// false=MysqlFulltext can't get value; signals to SearchResult::get() that it should try to load the field if it can
