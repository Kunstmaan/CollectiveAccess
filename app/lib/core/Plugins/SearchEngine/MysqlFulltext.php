...
class WLPlugSearchEngineMysqlFulltext extends WLPlug implements IWLPlugSearchEngine {
	# -------------------------------------------------------
	...
	public function setOption($ps_option, $pm_value) {
		if ($this->isValidOption($ps_option)) {
			...
			switch($ps_option) {
				case 'limit':
					//Zend_Search_MysqlFulltext::setResultSetLimit($pm_value);
					break;
			}
			return true;
		}
	...
	public function search($pn_subject_tablenum, $ps_search_expression, $pa_filters=array(), $po_rewritten_query=null) {
		...
		if (trim($ps_search_expression) === '((*))') {
			...
			if (is_array($va_filters = $this->getFilters()) && sizeof($va_filters)) {
				...
				if ($t_instance = $o_datamodel->getInstanceByTableNum($pn_subject_tablenum, true)) {
					foreach($va_filters as $va_filter) {
						...
						$vs_sql = "
							DELETE FROM ca_mysql_fulltext_search_final
							WHERE
								row_id IN (
