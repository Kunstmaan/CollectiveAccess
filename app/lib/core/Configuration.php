 ...
 */
	...
	/* ---------------------------------------- */
	static function load($ps_file_path="", $pb_dont_cache=false) {
		global $_CONFIG_INSTANCE_CACHE;
		if (!isset($_CONFIG_INSTANCE_CACHE[$ps_file_path]) || $pb_dont_cache) {
 ...
 */
	function Configuration ($ps_file_path="", $pb_die_on_error=true, $pb_dont_cache=false) {
		...
		#
		if (!isset($_CONFIG_CACHE[$this->ops_config_file_path]) || $pb_dont_cache) {
			...
			if (!isset($_CONFIG_CACHE[$vs_global_path])) {
				$this->loadFile($vs_global_path, false);
				$_CONFIG_CACHE[$vs_global_path] = $this->ops_config_settings;	// cache global.conf
			} else {
				$this->ops_config_settings = $_CONFIG_CACHE[$vs_global_path];
			}
			...
			if ($vb_loaded_config) {
				...
				$_CONFIG_CACHE[$this->ops_config_file_path] = $this->ops_config_settings;	
				if (is_object($vo_cache)) {
			...
			#
			$this->ops_config_settings = $_CONFIG_CACHE[$this->ops_config_file_path];
			return;
		}
	}
	/* ---------------------------------------- */
