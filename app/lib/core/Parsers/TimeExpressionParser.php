...
class TimeExpressionParser {
					...
					if ($va_date = $this->_parseDateExpression()) {
						...
						switch($va_token['type']) {
							...
							case TEP_TOKEN_ALPHA:
								...
								if (
									((in_array($va_next_token['value'], $this->opo_language_settings->getList("centuryIndicator"))) && (preg_match("/^([\d]+)(.*)$/", $va_token['value'], $va_matches)))
									||
									...
									//if (in_array($va_matches[2], $va_ordinals)) {
										...
										if (in_array($va_next_token['value'], $this->opo_language_settings->getList("centuryIndicator"))) {
											$va_next_token = null;
										}
			...
			switch($vn_state) {
				...
				case TEP_STATE_DATE_ELEMENT_GET_MONTH_NEXT:
					switch($va_token['type']) {
						...
						# ----------------------
						default:
							return $this->setParseError($va_token, TEP_ERROR_INVALID_DATE);
							break;
						# ----------------------
					}
					break;
			...
			switch($vn_state) {
					...
					if ($va_token['type'] == TEP_TOKEN_ALPHA) {
						if (preg_match("/^([\d\.]+)([dmyDMY]{1})$/", $va_token['value'], $va_matches)) {
							if (!is_numeric($va_matches[1])) {
								return $this->setParseError($va_token, TEP_ERROR_INVALID_UNCERTAINTY);
							} 
							...
							return $this->setParseError($va_peek, TEP_ERROR_INVALID_UNCERTAINTY);
						}
					} else {
						return $this->setParseError($va_peek, TEP_ERROR_INVALID_UNCERTAINTY);
					}
				...
				case TEP_STATE_DATE_GET_TIME:
					if ($va_time = $this->_parseTimeExpression()) {
						...
						return $this->setParseError($va_token, TEP_ERROR_INVALID_TIME);
					}
					break;
				# -------------------------------------------------------
				default:
					return $this->setParseError($va_token, TEP_ERROR_PARSER_ERROR);
					break;
				# -------------------------------------------------------
			}
		}
		
		return $this->setParseError($va_token, TEP_ERROR_INVALID_EXPRESSION);
	}
	# -------------------------------------------------------------------
	...
	function &getToken() {
		...
		$vs_token_lc = strtolower($vs_token);
		// today
		...
		switch(sizeof($va_tmp)) {
			...
			case 3:
				...
				if (($vn_month >= 1) && ($vn_month <= 12)) {
					if (($vn_day >= 1) && ($vn_day <= $this->daysInMonth($vn_month, $vn_year))) {
						if ($vn_year > 0) {
							return(array('value' => $vs_token, 'month' => $vn_month, 'day' => $vn_day, 'year' => $vn_year, 'type' => TEP_TOKEN_DATE));
						}
					}
					...
					if (($va_tmp[0] >= 1000) && ($va_tmp[0] <= 9999)) {
						$vn_year = $va_tmp[0]; $vn_month = $va_tmp[1]; $vn_day = $va_tmp[2];
						if (($vn_day >= 1) && ($vn_day <= $this->daysInMonth($vn_month, $vn_year))) {
							if (($vn_month >= 1) && ($vn_month <= 12)) {
								return(array('value' => $vs_token, 'month' => $vn_month, 'day' => $vn_day, 'year' => $vn_year, 'type' => TEP_TOKEN_DATE));
							}
	...
	function _processParseResults($pa_dates, $pa_options) {
		...
		if (!$pa_dates['start']['month'] && !$pa_dates['start']['year']) {
			...
			if ($vn_start > $vn_end) {
				return $this->setParseError(null, TEP_ERROR_RANGE_ERROR);
			}
			...
			if (!$pa_dates['start']['era'] && ($pa_dates['start']['year'] > 0) && ($pa_dates['start']['year'] <= 99)) {
				$va_tmp = getDate();
			...
			if (!$pa_dates['end']['era'] && ($pa_dates['end']['year'] > 0) && ($pa_dates['end']['year'] <= 99)) {
				$va_tmp = getDate();
				$vn_current_year = intval(substr($va_tmp['year'], 2, 2));		// get last two digits of current year
				$vn_current_century = intval(substr($va_tmp['year'], 0, 2)) * 100;
			...
			if ($pa_dates['end']['month'] === null) { $pa_dates['end']['month'] = $pa_dates['start']['month']; }
			...
			if (strlen($vn_start_uncertainty) > 9) {
				return $this->setParseError(null, TEP_ERROR_INVALID_UNCERTAINTY);
			}
			...
			if (strlen($vn_end_uncertainty) > 9) {
				return $this->setParseError(null, TEP_ERROR_UNCERTAINTY_OVERFLOW);
			}
		...
		if ($va_start_pieces['is_circa']) {
			...
			// dates within same month and year with differing uncertainties
							...
							if (
								...
								$va_end_pieces['day'] == $this->daysInMonth($va_end_pieces['month'], $va_end_pieces['year'])
							) {
			// month and year only
		...
		// years only
			...
			// catch decade dates
					...
					if ((($vs_start_year % 10) == 0) && ($vs_end_year == ($vs_start_year + 9))) {
						...
						return ($vs_prerange_conjunction ? $vs_range_conjunction.' ': '').$vs_start_year.' '.$vs_range_conjunction.' '.$vs_end_year;
					}
	...
	function _dateTimeToText($pa_date_pieces, $pa_options=null) {
		...
		if (!$pa_options['timeOmit']) {
			...
			$vs_time = $this->_timeToText($vn_seconds);
	...
	function getISODateTime($pa_date, $ps_mode='START') {
		if (
			...
			$vs_date = $pa_date['year'].'-'.$pa_date['month'].'-'.$pa_date['day'];
		} else {
