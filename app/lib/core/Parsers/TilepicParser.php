...
class TilepicParser {
	...
	function encode ($ps_filepath, $ps_output_path, $pa_options) {
		...
		switch($this->backend) {
			case LIBRARY_GD:
				...
				break;
			case LIBRARY_MAGICKWAND:
				return $this->encode_magickwand($ps_filepath, $ps_output_path, $pa_options);
				break;
			case LIBRARY_IMAGICK:
	...
	# ------------------------------------------------------------------------------------
	function encode_magickwand ($ps_filepath, $ps_output_path, $pa_options) {
		if (!($magick = $this->mimetype2magick[$pa_options["output_mimetype"]])) {
			$this->error = "Invalid output format";
			return false;
		}
		...
		#
		$h = NewMagickWand();
		MagickReadImage($h, $ps_filepath);
        if (WandHasException($h)) {
			$reason = 		WandGetExceptionType($h);
			$description = 	WandGetExceptionString($h);
			$this->error = "Couldn't open image $ps_filepath: $reason; $description";
			return false;
        }
        
        MagickSetImageType($h, MW_TrueColorType);

		if (!MagickSetImageColorspace( $h, MW_RGBColorspace)) {
			$reason      = WandGetExceptionType( $h ) ;
			$description = WandGetExceptionString( $h ) ;
			$this->error = "$reason: $description during RGB colorspace transformation operation";
			return false;
		}
        ...
        if ($pa_options["scale_factor"] != 1) {
			...
			if (!MagickResizeImage( $h, $image_width, $image_height, MW_CubicFilter, $pa_options["antialiasing"])) {
		...
		for($l=$pa_options["layers"]; $l >= 1; $l--) {
			...
			$layer_list[] = array();
			while($y < $image_height) {
				if (!($slice = MagickGetImageRegion($h, $wx, $wy, $x, $y))) {
					$reason      = WandGetExceptionType( $h ) ;
					$description = WandGetExceptionDescription( $h ) ;
					$this->error = "Couldn't create tile: $reason; $description";
					return false;
				}
				MagickSetCompressionQuality($slice, $pa_options["quality"]);
		...
		if (($layer_number > 0) && ($layer_number <= $this->properties['layers'])) {
			...
			switch($this->backend) {
				case LIBRARY_GD:
					...
					break;
				case LIBRARY_MAGICKWAND:
					$h = $this->getLayer_magickwand($layer_number, $magick);
					break;
				case LIBRARY_IMAGICK:
	...
	# ------------------------------------------------------------------------------------
	function getLayer_magickwand($layer_number, $magick) {
		$layer_tiles = $this->getFileGeometry();
		if (!($tile_count = $layer_tiles[$layer_number]['tiles'])) {
			$this->error = "Invalid file";
			return false;
