		...
		protected $ops_context = '';
 		...
 		function Index($pb_dont_render_view=false, $pa_options=null) {
			...
			// Get elements of result context
 			...
 			if ($this->opb_type_restriction_has_changed) { 
 		...
 		public function getFacet($pa_options=null) {
 			...
 			$vs_cache_key = md5(join("/", array($ps_facet_name.$vs_show_group.$vs_grouping)));
 			
 			if ($vs_content = $this->opo_browse->getCachedFacetHTML($vs_cache_key)) { 
 			...
 			$this->view->setVar('browse_last_id', $this->opo_result_context->getParameter($ps_facet_name.'_browse_last_id'));
