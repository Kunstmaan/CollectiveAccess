<?php
/** ---------------------------------------------------------------------
 * app/lib/ca/Attributes/Values/OccurrenceAttributeValue.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
$_ca_attribute_settings['OccurrenceAttributeValue'] = array(
	'fieldWidth' => array(
	...
	'restrictToOccurrenceTypeIdno' => array(
		'formatType' => FT_TEXT,
		'displayType' => DT_FIELD,
		'default' => '',
		'width' => 50, 'height' => 1,
		'label' => _t('Occurrence type restriction'),
		'description' => _t('Insert idno of a occurrence type here to restrict the lookup mechanism to that type. (The default is empty.)')
	),
...
class OccurrenceAttributeValue extends AttributeValue implements IAttributeValue {
	# ------------------------------------------------------------------
	private $opn_occurrence_id;
	private $ops_text;
	# ------------------------------------------------------------------
	...
	public function getOccurrenceID() {
 	...
 	public function parseValue($ps_value, $pa_element_info) {
		...
		if (!$ps_value) {
			if(intval($va_settings["canBeEmpty"])!=1){
				$this->postError(1970, _t('Entry was blank.'), 'OccurrenceAttributeValue->parseValue()');
				return false;
			}
			return array();
		} else {
			...
			if($va_tmp[1]){
				...
				if($this->opn_occurrence_id){ // odds are that the same value was saved again (e.g. due to changes in the same Container). problem is that we lost the original ID so we have to restore it here.
					return array(
						'value_integer1' => $this->opn_occurrence_id,
					);
				}
 	...
 	public function htmlFormElement($pa_element_info, $pa_options=null) {
		file_put_contents("/tmp/attr_test","FORMELEMENT OCC ID:".$this->getOccurrenceID()."\n",FILE_APPEND);
 		$o_config = Configuration::load();

 		$va_settings = $this->getSettingValuesFromElementArray($pa_element_info, array('fieldWidth'));

 		$vs_element = //'<div id="occ_'.$pa_element_info['element_id'].'_input{n}">'.
 			caHTMLTextInput(
				...
				array(
					...
					'maxlength' => 512,
					'id' => "occ_".$pa_element_info['element_id']."_autocomplete{n}"
				)
			).
			caHTMLHiddenInput(
 	...
 	public function getAvailableSettings() {
 		...
 		return $_ca_attribute_settings['OccurrenceAttributeValue'];
 	}
 	# ------------------------------------------------------------------
		/**
