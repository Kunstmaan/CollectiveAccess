...
require_once(__CA_LIB_DIR__.'/core/Configuration.php');
	...
	class Attribute {
 		...
 		# ------------------------------------------------------------------
 		public function getDisplayValues($pb_index_with_element_ids=false) {
 			if (!is_array($this->opa_values)) { return null; }
 			
 			$va_display_values = array();
 			foreach($this->opa_values as $o_val) {
 				$va_display_values[$pb_index_with_element_ids ? $o_val->getElementID() : $o_val->getElementCode()] = $o_val->getDisplayValue();
 			}
 		...
 		static public function getAttributeTypes() {
 			...
 			return $o_attribute_types->getList('types');
 		}
 		# ------------------------------------------------------------------
