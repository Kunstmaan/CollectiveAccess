 ...
 */
 	...
 	class ImportController extends ActionController {
 		...
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
			...
			// Get directory list
			$va_dir_list_with_file_counts = caGetSubDirectoryList($this->opo_config->get('importRootDirectory'));
			...
			if (!is_array($va_idno_values = $o_numbering_plugin->htmlFormValuesAsArray('idno', '', true))) { $va_idno_values = array(); }
 			...
 			if (!sizeof($va_errors)) {
 				$va_params = array(
					...
					'number_of_files'					=> sizeof(caGetDirectoryContentsAsList($ps_directory)),
					'user_id'							=> $this->request->getUserID()
				);
				
 				if ($o_config->get('queue_enabled')) {
