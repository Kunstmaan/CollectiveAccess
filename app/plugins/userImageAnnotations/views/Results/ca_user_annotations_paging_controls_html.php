 ...
 */
	...
	if($this->getVar('num_pages') > 1){
		...
		if ($this->getVar('page') > 1) {
			$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, 'find', $vs_controller_name, 'Index', $va_previous_link_params)."\"); return false;' class='button'>&lsaquo; Previous</a>";
		}
		$vs_searchNav .= '&nbsp;&nbsp;&nbsp;Page '.$this->getVar('page').'/'.$this->getVar('num_pages').'&nbsp;&nbsp;&nbsp;';
		if ($this->getVar('page') < $this->getVar('num_pages')) {
			$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, 'find', $vs_controller_name, 'Index', $va_next_link_params)."\"); return false;' class='button'>Next &rsaquo;</a>";
		}
		$vs_searchNav .= "</div>";
		$vs_searchNav .= '<form action="#">Jump to page: <input type="text" size="3" name="page" id="jumpToPageNum" value=""/> <a href="#" onclick=\'jQuery("#resultBox").load("'.caNavUrl($this->request, 'find', $vs_controller_name, 'Index', $va_jump_to_params).'/page/" + jQuery("#jumpToPageNum").val());\' class="button">GO</a></form>';
	}
