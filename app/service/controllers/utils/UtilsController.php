<?php
/* ----------------------------------------------------------------------
 * app/service/controllers/utils/UtilsController.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	class UtilsController extends BaseServiceController {
		# -------------------------------------------------------
		...
		public function soapWSDL(){
			$vs_service =
				$this->request->config->get("site_host").
				__CA_URL_ROOT__.
				"/service.php/utils/Utils/soap";
			$vo_autodiscover = new Zend_Soap_AutoDiscover(true,$vs_service);
			$vo_autodiscover->setClass('UtilsService',$this->request);
			$this->view->setVar("autodiscover",$vo_autodiscover);
			$this->render("utils_soap_wsdl.php");
		}
		# -------------------------------------------------------
		public function rest(){
			$vo_restserver = new Zend_Rest_Server();
			$vo_restserver->setClass('UtilsService',null,array($this->request));
			$this->view->setVar("rest_server",$vo_restserver);
			$this->render("utils_rest.php");
		}
		# -------------------------------------------------------
	}
?>
