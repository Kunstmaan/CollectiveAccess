...
BaseModel::$s_ca_models_definitions['ca_sets'] = array(
 	...
 	'FIELDS' 			=> array(
 		'set_id' => array(
				...
				'IS_NULL' => false, 
				'DEFAULT' => '',
				'LABEL' => 'Set id', 'DESCRIPTION' => 'Unique identifier for the set.'
		),
		'parent_id' => array(
...
class ca_sets extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider {
	 ...
	 *			row_id = 
	 * @return array A list of sets keyed by set_id and then locale_id. Keys for the per-locale value array include: set_id, set_code, status, public access, owner user_id, content table_num, set type_id, set name, number of items in the set (item_count), set type name for display and set content type name for display.
	 */
	public function getSets($pa_options=null) {
		...
		if ($pn_user_id) {
			...
			if ($t_user->getPrimaryKey()) {
				if (is_array($va_groups = $t_user->getUserGroups()) && sizeof($va_groups)) {
					...
					$va_sql_wheres[] = "((cs.user_id = ".intval($pn_user_id).") OR (cs.set_id IN (SELECT set_id FROM ca_sets_x_user_groups WHERE group_id IN (".join(',', array_keys($va_groups)).") {$vs_access_sql})))";
				} else {
					$va_sql_wheres[] = "(cs.user_id = ".intval($pn_user_id).")";
				}
			}
		}
		
		if (!is_null($pa_public_access) && is_array($pa_public_access) && sizeof($pa_public_access)) {
			$va_sql_wheres[] = "(cs.access = ".join(', ', $pa_public_access).")";
		}
		
		if (!is_null($pn_type_id)) {
			$t_list = new ca_lists();
			if ($va_type_info = $t_list->getItemFromList('set_types', $pm_type)) {
				$va_sql_wheres[] = "(cs.type_id = ".intval($va_type_info['item_id']).")";
			}
		}
		...
		// get set item counts
		$qr_res = $o_db->query("
			SELECT cs.set_id, count(*) item_count
			FROM ca_sets cs
	...
	public function isInSet($pm_table_name_or_num, $pn_row_id, $pm_set_code_or_id) {
		...
		$qr_res = $o_db->query("
			...
			INNER JOIN ca_set_items AS csi ON cs.set_id = csi.set_id
			WHERE
				(cs.set_id = ?) AND (csi.row_id = ?) AND (cs.table_num = ?)
	...
	public function haveAccessToSet($pn_user_id, $pn_access, $pn_set_id=null) {
		if ($pn_set_id) { 
			...
			$vn_set_user_id = $this->get('user_id');
		}
		if(!$vn_set_id && !($vn_set_id = $this->getPrimaryKey())) { 
			return true; // new set
		}
		...
		if (($this->get('access') > 0) && ($pn_access == __CA_SET_READ_ACCESS__)) {	 // public sets are readable by all
			return ca_sets::$s_have_access_to_set_cache[$vn_set_id.'/'.$pn_user_id.'/'.$pn_access] = true; 
		}
	...
	public function getRowIDRanks($pa_options=null) {
		...
		while($qr_res->nextRow()) {
			$va_row = $qr_res->getRow();
			$va_items[$qr_res->get('row_id')] = $qr_res->get('rank');
		}
		return $va_items;
	}
	# ------------------------------------------------------
...
LEFT JOIN ca_object_representations AS cor ON coxor.representation_id = cor.representation_id\n";
		...
		$qr_res = $o_db->query("
			...
			{$vs_rep_join_sql}
			WHERE
				casi.set_id = ? {$vs_rep_where_sql} {$vs_access_sql}
			ORDER BY 
				casi.rank ASC
			{$vs_limit_sql}
		", (int)$vn_set_id);
	 ...
	 * @return string Rendered HTML bundle for display
	 */
	public function getItemSetMembershipHTMLFormBundle($po_request, $ps_form_name, $pn_table_num, $pn_row_id, $pn_user_id=null) {
		$o_view = new View($po_request, $po_request->getViewsDirectoryPath().'/bundles/');
	...
	public static function getFirstItemsFromSets($pa_set_ids, $pa_options=null) {
		...
		foreach($pa_set_ids as $vn_set_id) {
			if ($t_set->load($vn_set_id)) {
				$va_item_list = caExtractValuesByUserLocale($t_set->getItems(array('limit' => 1, 'thumbnailVersion' => $vs_version)));
	...
	public function getRepresentationTags($ps_version, $pa_options=null) {
		...
		$qr_res = $o_db->query("
			...
			WHERE
				(casi.set_id = ?) AND (caxor.is_primary = 1)
				{$vs_access_sql}
			ORDER BY 
				casi.rank ASC
		", (int)$vn_set_id);
