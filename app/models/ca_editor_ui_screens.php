...
class ca_editor_ui_screens extends LabelableBaseModelWithAttributes {
	# ---------------------------------
	...
	# ----------------------------------------
	public function addBundlePlacement($ps_bundle_name, $ps_placement_code, $pa_settings) {
		...
		$t_placement->set('bundle_name', $ps_bundle_name); // todo: verify bundle name
		$t_placement->set('screen_id', $vn_screen_id);
		$t_placement->set('placement_code', $ps_placement_code);
		// add settings
		foreach($pa_settings as $vs_key => $vm_value) {
			if($t_placement->isValidSetting($vs_key)) {
				$t_placement->setSetting($vs_key, $vm_value);
			}
		}
	...
	# ----------------------------------------
	public function removeBundlePlacement($ps_bundle_name) {
		if (!($vn_screen_id = $this->getPrimaryKey())) { return null; }
	
	}
	# ----------------------------------------
	public function removeAllBundlePlacements() {
		if (!($vn_screen_id = $this->getPrimaryKey())) { return null; }
		$va_placements = $this->getBundlePlacements();
	}
	# ----------------------------------------
	public function getBundlePlacements() {
		if (!($vn_screen_id = $this->getPrimaryKey())) { return null; }
	...
	# ----------------------------------------
	/**
	 * Adds restriction (a binding between a specific item and optional type and the screen)
	 *
	...
	public function addTypeRestriction($pn_table_num, $pn_type_id, $va_settings) {
		...
		//if ($this->get('parent_id')) { return null; }						// screen must be root of hierarchy
		...
		if ($t_restriction->numErrors()) {
			...
			print_r($t_restriction->getErrors());
			return false;
		}
		return true;
	}
	# ----------------------------------------
	/**
	 * Remove type restrictions from this screen for specified table and, optionally, type
	 *
	 * $pn_table_num: the number of the table to bind to
	 * 
	 */
	public function removeTypeRestriction($pn_table_num, $pn_type_id=null) {
		...
		$qr_res = $o_db->query("
			...
			WHERE
				table_num = ? AND screen_id = ? AND {$vs_type_id_sql}
	...
	public function getTypeRestrictions($pn_table_num=null, $pn_type_id=null) {
		if (!($vn_screen_id = $this->getPrimaryKey())) { return null; }		// element must be loaded
