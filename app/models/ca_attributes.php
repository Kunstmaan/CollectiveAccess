...
class ca_attributes extends BaseModel {
	...
	public function addAttribute($pn_table_num, $pn_row_id, $pm_element_code_or_id, $pa_values) {
		...
		foreach($va_elements as $va_element) {
			...
			if (!$t_attr_val->addValue($vm_value, $va_element, $vn_attribute_id)) {
	...
	public function editAttribute($pa_values) {
		...
		foreach($va_attr_vals as $o_attr_val) {
			...
			if ($t_attr_val->load($o_attr_val->getValueID())) {
				...
				if (!$t_attr_val->editValue($vm_value)) {
	 ...
	 *			dontFetchAlreadyCachedValues = skip fetching attributes for row_ids that have already been fetched. This may increase performance in some situations. Default is false.
	 * @return boolean Always return true
	 */
	static public function prefetchAttributes($po_db, $pn_table_num, $pa_row_ids, $pa_options=null) {
		if (isset($pa_options['dontFetchAlreadyCachedValues']) && $pa_options['dontFetchAlreadyCachedValues']) {
			foreach($pa_row_ids as $vn_i => $vn_row_id) {
				if (isset(ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$vn_row_id])) {
					unset($pa_row_ids[$vn_i]);
				}
			}
		}
		...
		$qr_attrs = $po_db->query("
			...
			WHERE
				(caa.table_num = ?) AND (caa.row_id IN (?))
			ORDER BY
				caa.attribute_id
		", array((int)$pn_table_num, $pa_row_ids));
		if ($po_db->numErrors()) {
			return false;
		}
		$va_attrs = array();
		...
		while($qr_attrs->nextRow()) {
			...
			if ($vn_last_attribute_id != $va_raw_row['attribute_id']) {
				if ($vn_last_attribute_id && $vn_last_row_id) {
					$va_attrs[$vn_last_row_id][] = $o_attr;
					$vn_val_count == 0;
				}
		...
		foreach($va_attrs as $vn_row_id => $va_attrs_for_row) {
			unset($va_row_id_with_no_attributes[$vn_row_id]);
			ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$vn_row_id] = $va_attrs_for_row;
			
			// Limit cache size
			if (sizeof(ca_attributes::$s_get_attributes_cache) > 256) {
		...
		foreach($va_row_id_with_no_attributes as $vn_row_id => $vn_dummy) {
			ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$vn_row_id] = array();
	 ...
	 * Returns a list (indexed array) of Attribute objects.
	 */
	static public function getAttributes($po_db, $pn_table_num, $pn_row_id, $pa_options=null) {
		if (is_array(ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$pn_row_id])) {
			return ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$pn_row_id];
		}
		
		if (!is_array($pa_options)) { $pa_options = array(); }
		
		if (!(ca_attributes::prefetchAttributes($po_db, $pn_table_num, array($pn_row_id)))) {
			return null;
		}
	...
	static public function getAttributeValueForIDs($po_db, $pn_table_num, $pa_row_ids, $pn_element_id, $pa_options=null) {
		...
		if (!$vb_is_cached) {
			if (!(ca_attributes::prefetchAttributes($po_db, $pn_table_num, array($pa_row_ids)))) {
				return null;
			}
		}
		...
		$va_values = array();
		foreach($pa_row_ids as $vn_i => $vn_row_id) {
			foreach(ca_attributes::$s_get_attributes_cache[$pn_table_num.'/'.$vn_row_id] as $o_attr) {
