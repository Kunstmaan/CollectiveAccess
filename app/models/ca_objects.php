...
BaseModel::$s_ca_models_definitions['ca_objects'] = array(
 	...
 	'FIELDS' 			=> array(
		'object_id' => array(
				...
				'IS_NULL' => false, 
				'DEFAULT' => '',
				'LABEL' => 'Object id', 'DESCRIPTION' => 'Unique identifier for the object.'
		),
		'parent_id' => array(
		...
		'idno_sort' => array(
				...
				'IS_NULL' => false, 
				'DEFAULT' => '',
				'LABEL' => 'Sortable object identifier', 'DESCRIPTION' => 'Value used for sorting obejcts on identifier value.',
				'BOUNDS_LENGTH' => array(0,255)
		),
...
class ca_objects extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider {
	...
	public function delete($pb_delete_related=false){
 	...
 	public function getRepresentations($pa_versions=null, $pa_version_sizes=null, $pa_options=null) {
 		...
 		$qr_reps = $o_db->query("
 			SELECT caor.representation_id, caor.media, caoor.is_primary, caor.access, caor.status, l.name, caor.locale_id, caor.media_metadata, caor.type_id
 		...
 		while($qr_reps->nextRow()) {
			...
			if (isset($va_info['INPUT']['FETCHED_FROM']) && ($vs_fetched_from_url = $va_info['INPUT']['FETCHED_FROM'])) {
 			...
 			$va_tmp['num_multifiles'] = $t_rep->numFiles($qr_reps->get('representation_id'));
 			$va_reps[] = $va_tmp;
 		}
 		return $va_reps;
 	}
 	 ...
 	 * @param $pa_attributes - array of attributes to attach to new representation [NOT CURRENTLY IMPLEMENTED]
 	 ...
 	 */
 	public function addRepresentation($ps_media_path, $pn_type_id, $pn_locale_id, $pn_status, $pn_access, $pb_is_primary, $pa_attributes=null, $pa_options=null) {
 		if (!($vn_object_id = $this->getPrimaryKey())) { return null; }
 	 ...
 	 *
 	 */
 	public function editRepresentation($pn_representation_id, $ps_media_path, $pn_locale_id, $pn_status, $pn_access, $pb_is_primary, $pa_attributes=null, $pa_options=null) {
 		if (!($vn_object_id = $this->getPrimaryKey())) { return null; }
