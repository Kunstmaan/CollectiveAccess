...
class ca_search_forms extends BundlableLabelableBaseModelWithAttributes {
		...
		if ($pn_user_id) {
			...
			if ($t_user->getPrimaryKey()) {
				if (is_array($va_groups = $t_user->getUserGroups()) && sizeof($va_groups)) {
					...
					$vs_access_sql = "((sf.user_id = ".intval($pn_user_id).") OR (sf.form_id IN (SELECT form_id FROM ca_search_forms_x_user_groups WHERE group_id IN (".join(',', array_keys($va_groups)).") {$vs_access_sql})))";
				} else {
					$vs_access_sql = "(sf.user_id = ".intval($pn_user_id).")";
				}
				
				if ($pn_access == __CA_SEARCH_FORM_READ_ACCESS__) {
					$vs_access_sql = "({$vs_access_sql} OR (sf.is_system = 1))";
				}
	 ...
	 *
	 */
	 public function getFormCount($pm_table_name_or_num=null, $pn_user_id=null, $pa_options=null) {
	 	$o_dm = $this->getAppDatamodel();
	 	$vn_table_num = $o_dm->getTableNum($pm_table_name_or_num);
	 	if ($pm_table_name_or_num && !($vn_table_num = $o_dm->getTableNum($pm_table_name_or_num))) { return null; }
		...
		if ($pn_user_id) {
			if(isset($pa_options['includeUsableForms']) && $pa_options['includeUsableForms']) {
				$va_sql_wheres[] = "((sf.user_id = ".intval($pn_user_id).") OR (sf.is_system = 1))";
			} else {
				$t_user = new ca_users($pn_user_id);
				if ($t_user->canDoAction('is_administrator') || $t_user->canDoAction('can_manage_system_search_forms')) {
					$va_sql_wheres[] = "((sf.user_id = ".intval($pn_user_id).") OR (sf.is_system = 1))";
				} else {
					$va_sql_wheres[] = "(sf.user_id = ".intval($pn_user_id).")";
				}
			}
		}
	 ...
	 */
	public function getSearchFormHTMLFormBundle($po_request, $ps_form_name) {
	...
	public function getAvailableBundles($pm_table_name_or_num=null, $pa_options=null) {
				...
				// related table
					...
					foreach($va_fields['fields'] as $vs_field => $va_field_indexing_info) {
						...
						if (($va_field_info = $t_table->getFieldInfo($vs_field))) {
							...
							$vs_related_table = $t_table->getProperty('NAME_PLURAL');
							if (method_exists($t_table, 'getSubjectTableInstance')) {
								$t_subject = $t_table->getSubjectTableInstance();
								$vs_related_table = $t_subject->getProperty('NAME_PLURAL');
							}
							
							$vs_display = $t_table->getDisplayLabel($vs_bundle);
							if  (method_exists($t_table, "getSubjectTableName") && ($vs_primary_table == $vs_subject_table)) {
								$vs_label = "{$vs_display} (".$t_subject->getProperty('NAME_SINGULAR').")";
							} else {
								$vs_label = "{$vs_display} ("._t('for related %1', $vs_related_table).")";
							}
							
							$va_available_bundles[$vs_display][$vs_bundle] = array(
								'bundle' => $vs_bundle,
								'display' => $vs_label,
								'description' => $t_table->getDisplayDescription($vs_bundle),
								'settingsForm' => $t_placement->getHTMLSettingForm(array('id' => $vs_bundle.'_0')),
								'settings' => $va_additional_settings
							);
						}
		...
		foreach($va_access_points as $vs_access_point => $va_access_point_info) {
			...
			$va_available_bundles[($vs_display = (isset($va_access_point_info['name']) && $va_access_point_info['name'])  ? $va_access_point_info['name'] : $vs_access_point)][$vs_access_point] = array(
				'bundle' => $vs_access_point,
				'display' => $vs_display,
				'description' =>  ((isset($va_access_point_info['description']) && $va_access_point_info['description'])  ? $va_access_point_info['description'] : ''),
	...
	public function getPlacementsInForm($pa_options=null) {
		...
		foreach($va_placements as $vn_placement_id => $va_placement) {
			$va_placement['display'] = $va_available_bundles[$va_placement['bundle_name']]['display'];
				...
				if (isset($va_access_points[$va_tmp[0]])) {
					...
					$va_output[] = array(
							'element' => caHTMLTextInput($vs_field, array(
								...
								'height' => (isset($va_element['settings']['height']) && ($va_element['settings']['width'] > 0)) ? $va_element['settings']['height'] : 1, 
