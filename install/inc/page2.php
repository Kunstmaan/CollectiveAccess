...
<?php
	...
	if($vo_installer->numErrors()){
		...
		if($vo_installer->numErrors()){
			...
			caIncrementProgress($vn_progress, "Creating administrator login");
			$vs_pw = $vo_installer->createAdminAccount();
			...
			if($vo_installer->numErrors()){
				...
				caSetMessage("<div class='contentSuccess'><span style='font-size:18px;'><b>Installation was successful!</b></span><br/>You can now <a href='../index.php?action=login'>login</a> with username <span class='contentHighlight'>administrator</span> and password <span class='contentHighlight'>{$vs_pw}</span>.<br/><b>Make a note of this password!</b><br/><span style='font-size:11px;'>{$vs_time}</span></div>");
			}
		}
	}
?>
...
<?php
	...
	function caSetMessage($ps_message) {
		$ps_message = addslashes($ps_message);
		print "<script type='text/javascript'>jQuery('#installerLog').html('{$ps_message}');</script>";
		caFlushOutput();
	}
