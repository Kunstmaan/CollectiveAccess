...
class Installer {
	...
	public function processLists($f_callback=null){
		...
		foreach($va_lists as $vo_list){
			...
			$t_list->set("is_system_list",$vb_system);
		...
		foreach($va_uis as $vs_ui_code => $vo_ui) {
			...
			foreach($vo_ui->screens->children() as $vo_screen) {
				...
				foreach($vo_screen->bundlePlacements->children() as $vo_placement) {
					...
					$t_ui_screens->addBundlePlacement((string)$vo_placement->bundle, $vs_placement_code, $va_settings);
				}
				...
				if($vo_screen->typeRestrictions){
					foreach($vo_screen->typeRestrictions->children() as $vo_restriction){
						...
						$t_ui_screens->addTypeRestriction($vn_type, ($vs_restriction_type ? $t_list_item->getPrimaryKey(): null), array());
	...
	public function processSearchForms(){
		...
		foreach($va_forms as $vo_form){
			...
			$t_form->set("form_code",$vs_form_code);
			$t_form->set("is_system",$vb_system);
			$t_form->set("table_num",$vo_dm->getTableNum($vs_table));
	...
	public function processGroups(){
		...
		if ($t_user_group->numErrors()) {
			print "<div class='installTaskFailure'> failed! <span class='installTaskFailureInfo'>".join("; ",$t_user_group->getErrors())." creating user group hierarchy root</p><p>Installation halted</p></span></div>\n";
			$vb_fatal_error = true;
			break;
		}
		if($this->ops_base_name){ // "merge" profile and its base
			...
			foreach($po_settings_node->children() as $vo_setting) {
				...
				if ($vn_locale_id) { 
					...
					if (!isset($va_settings[$vs_setting_name])) {
						...
						if (!is_array($va_settings[$vs_setting_name] = $vs_value)) {
							$va_settings[$vs_setting_name] = array($vs_value);
						}
						$va_settings[$vs_setting_name][] = $vs_value;
					}
				}
