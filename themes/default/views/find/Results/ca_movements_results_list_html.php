<?php
/* ----------------------------------------------------------------------
 * themes/default/views/find/ca_movements_list_html.php 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
		...
		while(($vn_item_count < $vn_items_per_page) && $vo_result->nextHit()) {
			$vn_movement_id = $vo_result->get('movement_id');

			if($vo_ar->userCanAccess($this->request->user->getUserID(), array("editor","movements"), "MovementEditor", "Edit", array("movement_id" => $vn_movement_id))){
				$vs_action = "Edit";
			} else {
				$vs_action = "Summary";
			}
...
?>
			<tr <?php print ($i ==1) ? "class='odd'" : ""; ?>>
				<td style="width:10px">
					<input type='checkbox' name='add_to_set_ids' value='<?php print (int)$vn_movement_id; ?>' class="addItemToSetControl" />
				</td>
<?php
				foreach($va_display_list as $vn_placement_id => $va_display_item) {
					print "<td>".$t_display->getDisplayValue($vo_result, $vn_placement_id)."</td>";
				}
				print "<td style='width:5%;'>".caNavLink($this->request, caNavIcon($this->request, __CA_NAV_BUTTON_EDIT__), '', 'editor/movements', 'MovementEditor', $vs_action, array('movement_id' => $vn_movement_id))."</td>";
?>	
			</tr>
<?php
			$i++;
