		...
		that.scrollToImage = function(offset, dontUseEffects) {
			...
			for(i=minImageImage; i <= maxImageIndex; i++) {
				if (jQuery("#" + that.scrollingImageIDPrefix + i).length == 0) {
					...
					jQuery('#' + that.container).append('<div class="' + that.scrollingImageClass + '" id="' + that.scrollingImageIDPrefix + i + '" style="'+horizCentering + ' ' + vertCentering +'"><a href="' + that.scrollingImageList[i].link + '" '+(that.scrollingImageList[i].onclick ? 'onclick="' + that.scrollingImageList[i].onclick + '"' : '') + ' '+(that.scrollingImageList[i].rel ? 'rel="' + that.scrollingImageList[i].rel + '"' : '') + '><img src="' + that.scrollingImageList[i].url+ '" width="' + that.scrollingImageList[i].width + '" height ="' + that.scrollingImageList[i].height + '" border=\'0\'></a></div>');
